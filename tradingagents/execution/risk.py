...
class CooldownGuard:
    """Per-symbol cooldown backed by a JSON file on disk.
    ...
    flat ``{symbol: last_order_unix_ts}`` map.
    ...
    def can_trade(self, symbol: str, cooldown_seconds: float) -> tuple[bool, float | None]:
        """Return ``(allowed, remaining_seconds)``.

        ``remaining_seconds`` is None when allowed, otherwise the wait time
        before the next order on ``symbol`` is permitted.
        """
    ...
    def record(self, symbol: str) -> None:
        """Mark an order on ``symbol`` as just placed (now)."""
        state = self._load()
